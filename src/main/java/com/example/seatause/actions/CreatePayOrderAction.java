...
@Service
public class CreatePayOrderAction implements Action<TradeOrderStatusEnum, TradeOrderEventEnum> {
    @Override
    public void execute(StateContext<TradeOrderStatusEnum, TradeOrderEventEnum> stateContext) {
        System.out.println("支付单已创建，待支付");
        stateContext.getExtendedState().getVariables().put(MachineConstant.NEED_NEXT_STEP, true);
        stateContext.getExtendedState().getVariables().put(MachineConstant.NEXT_EVENT, TradeOrderEventEnum.PAYING);
    }
}
